...
for name, code in goods.items():
    lamps_quantity = 0
    lamps_cost = 0
    for story in store[code]:
        lamps_quantity += story['quantity']
        lamps_cost += story['quantity'] * story['price']
    print('Товар', name, 'под номером:', code, 'кол-во', lamps_quantity, 'шт.', lamps_cost, 'руб')
